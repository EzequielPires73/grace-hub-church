...
export function TableDefault({ columns, data, forwardLink, actions }: Props) {
    ...
    return (
        <div className="flex flex-col gap-2 w-full">
            <header className="h-12 w-full bg-blue-900 rounded-md flex gap-4 px-4 items-center text-white">
                {
                    columns.map(column => (
                        <div key={column.id} className={`flex text-sm font-medium`} style={{ width: column.size }}>{column.name}</div>
                    ))
                }
            ...
            <div className="border border-gray-300 w-full rounded-md overflow-hidden">
                ...
                {data.map((item, index) => (
                    <div onClick={() => { forwardLink && router.push(forwardLink(item?.id)) }} key={item?.id ?? index} className={`h-12 w-full flex gap-4 px-4 items-center bg-white ${forwardLink && 'hover:bg-gray-100 cursor-pointer'} transition-all text-gray-800 ${index < data.length - 1 && 'border-b'}`}>
                        {
                            columns.map(column => column.format ? (
                                <div key={column.id} className={`text-sm font-medium line-clamp-1 text-ellipsis`} style={{ width: column.size }}>
                                    {column.format(column.subidentifier && column.identifier ? item[column.identifier][column.subidentifier] : item[column.identifier])}
                                </div>
                            )
                                : (
                                    <div key={column.id} className={`text-sm font-medium line-clamp-1 text-ellipsis`} style={{ width: column.size }}>
                                        {column.subidentifier && column.identifier ? item[column.identifier][column.subidentifier] : item[column.identifier]}
                                    </div>
                                ))
                        }
                        {actions &&
